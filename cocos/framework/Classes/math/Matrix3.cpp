...
namespace framework
{
	namespace math
	{
		const Real Matrix3::EPSILON = static_cast<Real>(1e-06);
		const Matrix3 Matrix3::ZERO(0,0,0,0,0,0,0,0,0);
		const Matrix3 Matrix3::IDENTITY(1,0,0,0,1,0,0,0,1);
		const Real Matrix3::ms_fSvdEpsilon = Real_Epsilon;
		const unsigned int Matrix3::ms_iSvdMaxIterations = 32;

		//-----------------------------------------------------------------------
		Vector3 Matrix3::GetColumn (size_t iCol) const
		{
			CCASSERT( 0 <= iCol && iCol < 3, "column count can not > 4" );
			...
			Matrix3& kR)
		{
			...
			{
				fSign = (kA[0][0] > Real_Zero ? Real_One : -Real_One);
				fT1 = kA[0][0] + fSign*fLength;
				fInvT1 = Real_One/fT1;
				afV[1] = kA[1][0]*fInvT1;
				afV[2] = kA[2][0]*fInvT1;

				fT2 = -Real_Two/(Real_One+afV[1]*afV[1]+afV[2]*afV[2]);
				...
				kL[1][1] = Real_One+fT2*afV[1]*afV[1];
				kL[1][2] = kL[2][1] = fT2*afV[1]*afV[2];
				kL[2][2] = Real_One+fT2*afV[2]*afV[2];
				bIdentity = false;
			}
			else
			{
			...
			if ( fLength > Real_Zero )
			{
				fSign = (kA[0][1] > Real_Zero ? Real_One : -Real_One);
				fT1 = kA[0][1] + fSign*fLength;
				afV[2] = kA[0][2]/fT1;

				fT2 = -Real_Two/(Real_One+afV[2]*afV[2]);
				...
				kR[0][0] = Real_One;
				kR[0][1] = kR[1][0] = Real_Zero;
				kR[0][2] = kR[2][0] = Real_Zero;
				kR[1][1] = Real_One+fT2;
				kR[1][2] = kR[2][1] = fT2*afV[2];
				kR[2][2] = Real_One+fT2*afV[2]*afV[2];
			}
			else
			{
				kR = Matrix3::IDENTITY;
			...
			if ( fLength > Real_Zero )
			{
				fSign = (kA[1][1] > Real_Zero ? Real_One : -Real_One);
				...
				Real fA = Real_One+fT2;
				Real fB = fT2*afV[2];
				Real fC = Real_One+fB*afV[2];

				if ( bIdentity )
				{
					kL[0][0] = Real_One;
					kL[0][1] = kL[1][0] = Real_Zero;
					kL[0][2] = kL[2][0] = Real_Zero;
					kL[1][1] = fA;
					kL[1][2] = kL[2][1] = fB;
					kL[2][2] = fC;
				}
			...
			Matrix3& kR)
		{
			...
			Real fDiscr = Math::Sqrt(fDiff*fDiff+static_cast<Real>(4.0)*fT12*fT12);
			Real fRoot1 = Real_Half*(fTrace+fDiscr);
			Real fRoot2 = Real_Half*(fTrace-fDiscr);
			...
			for (unsigned int i = 0; i < ms_iSvdMaxIterations; i++)
			{
				...
				if ( bTest1 )
				{
					...
					else
					{
						...
						fTan0 = Real_Half*(fTmp+Math::Sqrt(fTmp*fTmp + static_cast<Real>(4.0)));
						fCos0 = Math::InvSqrt(Real_One+fTan0*fTan0);
						fSin0 = fTan0*fCos0;
						...
						fCos1 = Math::InvSqrt(Real_One+fTan1*fTan1);
						fSin1 = -fTan1*fCos1;
		...
		Real Matrix3::MaxCubicRoot (Real afCoeff[3])
		{
			...
			const Real fOneThird = Real_One/static_cast<Real>(3.0);
			const Real fEpsilon = Real_Epsilon;
			Real fDiscr = afCoeff[2]*afCoeff[2] - static_cast<Real>(3.0)*afCoeff[1];
			if ( fDiscr <= fEpsilon )
			...
			if ( fPoly < Real_Zero )
			{
				...
				Real fTmp = Real_One+Math::Abs(afCoeff[1]);
				if ( fTmp > fX )
					fX = fTmp;
				fTmp = Real_One+Math::Abs(afCoeff[2]);
				if ( fTmp > fX )
					fX = fTmp;
			}

			// Newton's method to find root
			Real fTwoC2 = Real_Two*afCoeff[2];
			for (int i = 0; i < 16; i++)
			{
				...
				Real fDeriv = afCoeff[1]+fX*(fTwoC2+static_cast<Real>(3.0)*fX);
				fX -= fPoly/fDeriv;
			}
		...
		Real Matrix3::SpectralNorm () const
		{
			Matrix3 kP;
			size_t iRow, iCol;
			Real fPmax = Real_Zero;
			for (iRow = 0; iRow < 3; iRow++)
			{
				for (iCol = 0; iCol < 3; iCol++)
				{
					kP[iRow][iCol] = Real_Zero;
					for (int iMid = 0; iMid < 3; iMid++)
					{
						kP[iRow][iCol] +=
		...
		void Matrix3::ToAxisAngle (Vector3& rkAxis, Radian& rfRadians) const
		{
			...
			Real fCos = Real_Half*(fTrace-Real_One);
			rfRadians = Math::ACos(fCos);  // in [0,PI]

			if ( rfRadians > Radian(Real_Zero) )
			{
				...
				else
				{
					...
					if ( m[0][0] >= m[1][1] )
					{
						...
						if ( m[0][0] >= m[2][2] )
						{
							...
							rkAxis.x = Real_Half*Math::Sqrt(m[0][0] -
								m[1][1] - m[2][2] + Real_One);
							fHalfInverse = Real_Half/rkAxis.x;
						...
						else
						{
							...
							rkAxis.z = Real_Half*Math::Sqrt(m[2][2] -
								m[0][0] - m[1][1] + Real_One);
							fHalfInverse = Real_Half/rkAxis.z;
					...
					else
					{
						...
						if ( m[1][1] >= m[2][2] )
						{
							...
							rkAxis.y = Real_Half*Math::Sqrt(m[1][1] -
								m[0][0] - m[2][2] + Real_One);
							fHalfInverse  = Real_Half/rkAxis.y;
		...
		void Matrix3::FromAxisAngle (const Vector3& rkAxis, const Radian& fRadians)
		{
			...
			Real fOneMinusCos = Real_One-fCos;
			...
			Radian& rfRAngle) const
		{
			...
			if ( rfPAngle < Radian(Math::HALF_PI) )
			{
				...
				else
				{
					...
					rfRAngle = Radian(Real_Zero);  // any angle works
			...
			const Radian& fRAngle)
		{
			...
			Matrix3 kXMat(Real_One,Real_Zero,Real_Zero,Real_Zero,fCos,-fSin,Real_Zero,fSin,fCos);

			fCos = Math::Cos(fPAngle);
			fSin = Math::Sin(fPAngle);
			Matrix3 kYMat(fCos,Real_Zero,fSin,Real_Zero,Real_One,Real_Zero,-fSin,Real_Zero,fCos);

			fCos = Math::Cos(fRAngle);
			fSin = Math::Sin(fRAngle);
			Matrix3 kZMat(fCos,-fSin,Real_Zero,fSin,fCos,Real_Zero,Real_Zero,Real_Zero,Real_One);
		...
		void Matrix3::Tridiagonal (Real afDiag[3], Real afSubDiag[3])
		{
			...
			if ( Math::Abs(fC) >= EPSILON )
			{
				...
				fB *= fInvLength;
				fC *= fInvLength;
				Real fQ = Real_Two*fB*fE+fC*(fF-fD);
				afDiag[1] = fD+fC*fQ;
				afDiag[2] = fF-fC*fQ;
				afSubDiag[0] = fLength;
		...
		bool Matrix3::QLAlgorithm (Real afDiag[3], Real afSubDiag[3])
		{
			...
			for (int i0 = 0; i0 < 3; i0++)
			{
				...
				for (iIter = 0; iIter < iMaxIter; iIter++)
				{
					...
					Real fTmp0 = (afDiag[i0+1]-afDiag[i0])/(Real_Two*afSubDiag[i0]);
					Real fTmp1 = Math::Sqrt(fTmp0*fTmp0+Real_One);
					if ( fTmp0 < Real_Zero )
					...
					for (int i2 = i1-1; i2 >= i0; i2--)
					{
						...
						{
							fCos = fTmp0/fTmp3;
							fTmp1 = Math::Sqrt(fCos*fCos+Real_One);
						...
						{
							fSin = fTmp3/fTmp0;
							fTmp1 = Math::Sqrt(fSin*fSin+Real_One);
							afSubDiag[i2+1] = fTmp0*fTmp1;
							fCos = Real_One/fTmp1;
							fSin *= fCos;
						}
						fTmp0 = afDiag[i2+1]-fTmp2;
						fTmp1 = (afDiag[i2]-fTmp0)*fSin+Real_Two*fTmp4*fCos;
						fTmp2 = fSin*fTmp1;
