...
namespace framework
{
	...
	//	const Real Math::PI = Real( 4.0 * atan( Real_One ) );
 ...
 //   
	...
	//		zeroDot[0] = Math::RealEqual(dot[0], Real_Zero, static_cast<Real>(1e-3));
	...
	//		zeroDot[1] = Math::RealEqual(dot[1], Real_Zero, static_cast<Real>(1e-3) );
	...
	//		zeroDot[2] = Math::RealEqual(dot[2], Real_Zero, static_cast<Real>(1e-3));
	//		// Compare signs (ignore colinear / coincident points)
...
//	}
		...
		Radian Math::Sqrt(const Radian& fValue)
		{
			return Radian(sqrt(fValue.valueRadians()));
		}
		Degree Math::Sqrt(const Degree& fValue)
		{
