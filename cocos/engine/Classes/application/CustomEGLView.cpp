...
EGLView* CustomEGLView::s_pEglView = nullptr;
CustomEGLView::CustomEGLView()
: _captured(false)
...
bool CustomEGLView::init(const char* viewName, float width, float height, float frameZoomFactor)
{
...
void CustomEGLView::setAccelerometerKeyHook( LPFN_ACCELEROMETER_KEYHOOK lpfnAccelerometerKeyHook )
{
    _lpfnAccelerometerKeyHook=lpfnAccelerometerKeyHook;
}


bool CustomEGLView::isOpenGLReady()
{
    return nullptr != _mainWindow;
}

void CustomEGLView::end()
{
    if(_mainWindow)
        glfwSetWindowShouldClose(_mainWindow,1);
}

void CustomEGLView::swapBuffers()
{
    if(_mainWindow)
        glfwSwapBuffers(_mainWindow);
}

bool CustomEGLView::windowShouldClose()
{
	if(_mainWindow)
...
void CustomEGLView::pollEvents()
{
    glfwPollEvents();
}

void CustomEGLView::setIMEKeyboardState(bool /*bOpen*/)
{

}

void CustomEGLView::setFrameZoomFactor(float fZoomFactor)
{
    _frameZoomFactor = fZoomFactor;
    Director::getInstance()->setProjection(Director::getInstance()->getProjection());
}

float CustomEGLView::getFrameZoomFactor()
{
    return _frameZoomFactor;
}

void CustomEGLView::setFrameSize(float width, float height)
{
    EGLViewProtocol::setFrameSize(width, height);
}

void CustomEGLView::setViewPortInPoints(float x , float y , float w , float h)
{
...
void CustomEGLView::setScissorInPoints(float x , float y , float w , float h)
{
    float frameZoomFactorX = _frameBufferSize[0]/_screenSize.width;
    float frameZoomFactorY = _frameBufferSize[1]/_screenSize.height;
...
EGLView* CustomEGLView::getInstance()
{
