...
bool CCModelMd2::load(const char * szFilename)
{
    ...
    ucpBuffer = CCFileUtils::sharedFileUtils()->getFileData(fullPath.c_str(), "rb", (long*)&iFileSize);
...
void CCModelMd2::animate(float fSpeed, unsigned int uiStartFrame, unsigned int uiEndFrame, bool bLoop)
{
	...
	uiTime = (unsigned int)(float(tp.tv_sec - oldTime.tv_sec) + float(tp.tv_usec - oldTime.tv_usec) / 1000.0f);
